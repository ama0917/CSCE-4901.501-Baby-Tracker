...
try:
    ...
    # Fallback to default local file in backend folder
    if not cred:
...
def backup_data():
    try:
        ...
        for collection_name in collections_to_backup:
            # Create a table for each collection (quoted names for safety)
            cursor.execute(f'''
        ...
        print(f"[Backup] JSON backup created at {backup_json_path}")
